...
func (g *Graph) TopoShortestPath(L []Vertex) (distTo map[interface{}]int, edgeTo map[interface{}]interface{}) {
	/*
	...
	*/
	...
	edgeTo = map[interface{}]interface{}{}

	// For each vertex u in L
	for _, u := range L {
		// Walk through all neighbors v of u;
		for vh, weight := range g.adjacencyOut[u] {
			// x = dist(u) + w(u, v)
			x := distTo[u] + weight
